...
class JiraMetricsProject:
    ...
    def calculate_wip(self, sprint_start, num_sprints=6):
        """
        Counts tickets still in progress (not resolved) at each sprint boundary.
        """
        ...
        for idx, (start, end) in enumerate(sprints, 1):
            ...
            for issue in issues:
                ...
                # WIP means: no resolution date AND status not Done
                if not res_date and status.lower() not in ["done", "closed", "resolved"]:
                    if sprint_key not in self.results[category]["WIP"]:
                        self.results[category]["WIP"][sprint_key] = {}
                    self.results[category]["WIP"][sprint_key][group_key] = \
                        self.results[category]["WIP"][sprint_key].get(group_key, 0) + 1

    def display_report(self):
        print("\nPMO AUTOMATED METRICS - WIP REPORT")

        for category in ["Stories", "Bugs", "Story Bugs"]:
            print(f"\n=== {category.upper()} ===")

            print(f"\n[METRIC: WORK IN PROGRESS BY SPRINT]")
            if not self.results[category]["WIP"]:
                print(" - No WIP data calculated yet.")
            else:
                for sprint in sorted(self.results[category]["WIP"].keys(),
                                     key=lambda x: int(x.split()[1])):
                    groups = self.results[category]["WIP"][sprint]
                    print(f"\n{sprint}:")
                    for group, count in groups.items():
                        print(f" • {group:<35}: {count} ticket(s) in progress")
...
project.calculate_wip(sprint_start, num_sprints=6)
